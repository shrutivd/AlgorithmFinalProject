...
def tsp(data):
    ...
    if ENABLE_PLOT:
        # Show how the minimum spanning tree grows
        lines = []
        for node in MSTree:
            lines.append([tuple(data[node[0]]), tuple(data[node[1]])])

        collection = collections.LineCollection(lines, linestyles='--', linewidths=2)
        x = []
        y = []
        for line in lines:
            x.append(line[0][0])
    ...
    showTour(data,eulerian_tour,label='Eulerian')
...
def showTour(coordinates, path, label='Best Route'):
    ...
    fig, ax = pl.subplots()
    ax.plot(x,y,'--go',label=label,linewidth=2.5)

    for i in range(len(path)-1):
        ax.annotate(i+1, (x[i], y[i]), fontsize=15)
    pl.show()

def getCoordinatesByNumberOfCities(n, minCoordinate=1, maxCoordinate=100):
    coordinates = []
    for i in range(n):
        coordinates.append([random.randint(minCoordinate,maxCoordinate), random.randint(minCoordinate,maxCoordinate)])
    return coordinates

# coordinates = [[1380, 939], [2848, 96], [3510, 1671], [457, 334], [3888, 666], [984, 965], [2721, 1482], [1286, 525],
...
#coordinates = [[0,1],[30,20],[60,10],[70,45],[150,-10],[100,25],[160,110],[50,60],[80,90],[15,120]]
#length, path = tsp(coordinates)
trialNumberOfCities = list(range(10,100,20))

timing = []
for n in trialNumberOfCities:
    coordinates = getCoordinatesByNumberOfCities(n)
    start = time.perf_counter()
    length, path = tsp(coordinates)
    end = time.perf_counter()
    timing.append((end-start)*1000)
pl.plot(trialNumberOfCities, timing)
pl.show()

showTour(coordinates, path)